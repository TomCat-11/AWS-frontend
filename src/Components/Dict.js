  ...
  const filteredData = categoryData.filter((item) =>
    ...
    const fetchCategoryData = async (category) => {
        try {
          ...
          const endpoint = `https://13.53.129.50/ItemDataByCategory/${category}`;
      ...
      const deleteItem = async (itemId) => {
        try {
            ...
            const response = await axios.delete(`https://13.53.129.50/items/${itemId}`);
    ...
    <Card.Body>
      ...
      {filteredData.length > 0 ? (
        ...
        playsInline
                    ...
                    >
                      <source src={`https://13.53.129.50/${item.file}`} type="video/mp4" />
