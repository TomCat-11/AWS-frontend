...
const [newPassword, setNewPassword] = useState('');
  ...
  const [email, setEmail] = useState('');
    ...
    const sendOtp = async (e) => {
        ...
        try {
            const response = await axios.post('https://13.53.129.50:443/send-otp', { email });
    ...
    e.preventDefault();
        ...
        try {
            const response = await axios.post('https://13.53.129.50:443/verify-otp', { enteredOtp: otp, generatedOtp });
  ...
  const sendUserDatatoDB = async ()=>{
    if (validate()) {
      try {
        const response = await axios.post('https://13.53.129.50:443/Newuser', {
          username: name,
          email,
          password,
          phoneNumber,
...
const handleLogin = async (email, password) => {
    try {
        const response = await axios.post('https://13.53.129.50:443/login', { email, password });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put('https://13.53.129.50:443/update-password', {
        email,
